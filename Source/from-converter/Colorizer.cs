...
//ORIGINAL LINE: item = stack[stack_p - 1];
	  item.CopyFrom(stack[stack_p - 1]);
	}

	if (!item.override)
	{
	  uint64_t colorName = new uint64_t();
	  ...
	  {
		uint64_t color = new uint64_t();
		if (colorByName.get(color, new uint64_t(colorName)))
		{
		  item.colorName = (string)colorName;
		  item.color = uint32_t(color);
		}
		else if (!naggedName.get(new uint64_t(colorName)))
		{
		  naggedName.insert(new uint64_t(colorName), 1);
		  logger(1, "Unrecognized color name %s", (string)colorName);
		}
	  }
...
//C++ TO C# CONVERTER TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//ORIGINAL LINE: geometry->color = stack[stack_p - 1].color;
	geometry.color.CopyFrom(stack[stack_p - 1].color);
  }
  ...
  public override void attribute(string key, string val)
  {
	...
	if (key == colorAttribute)
	{
	  ...
	  else if (!naggedName.get(uint64_t(val)))
	  {
		...
		logger(1, "A Unrecognized color name %s", (string)val);
	  }
	}
  }
  ...
  private class StackItem
  {
	...
	public uint32_t color = new uint32_t();
	public bool override;
  }

  private Arena arena = new Arena();
  private Map colorNameByMaterialId = new Map();
  private Map colorByName = new Map();
  private Map naggedMaterialId = new Map();
  private Map naggedName = new Map();
  private Logger logger;
  private StackItem[] stack = null;
  private uint32_t stack_p = 0;
